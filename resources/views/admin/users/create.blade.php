...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="skin_image">{{ trans('cruds.user.fields.skin_image') }}</label>
                <div class="needsclick dropzone {{ $errors->has('skin_image') ? 'is-invalid' : '' }}" id="skin_image-dropzone">
                </div>
                @if($errors->has('skin_image'))
                    <span class="text-danger">{{ $errors->first('skin_image') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.skin_image_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    Dropzone.options.skinImageDropzone = {
    ...
    success: function (file, response) {
      $('form').find('input[name="skin_image"]').remove()
      $('form').append('<input type="hidden" name="skin_image" value="' + response.name + '">')
    },
    ...
    init: function () {
@if(isset($user) && $user->skin_image)
      var file = {!! json_encode($user->skin_image) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="skin_image" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
